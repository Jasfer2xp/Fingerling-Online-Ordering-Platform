 ...
 * 
 * Usage: 
 * 1. Upload this file to your Hostinger public_html root (same directory as config folder)
 * 2. Access via browser: https://fingerling.shop/test_auto_message.php?order_id=123
 * 3. Replace 123 with an actual paid order ID
 */

// On Hostinger, files are directly in public_html, not in a capstone subdirectory
// Try current directory first (if script is in same location as config)
$base_path = __DIR__;

// If config not found in current directory, try capstone subdirectory (for localhost testing)
if (!file_exists($base_path . '/config/config.php')) {
    if (file_exists($base_path . '/capstone/config/config.php')) {
        $base_path = $base_path . '/capstone';
    } else {
        die("Error: Cannot find config.php.<br>Current directory: " . __DIR__ . "<br>Please ensure this script is in the same directory as your config folder.");
    }
}

require_once $base_path . '/config/config.php';
require_once $base_path . '/config/database.php';
require_once $base_path . '/includes/auto_payment_message.php';
    ...
    try {
        $recent_orders = $database->fetchAll(
            "SELECT o.id, o.order_number, o.status,
             ...
             WHERE o.status IN ('paid', 'confirmed', 'confirmed_and_paid', 'preparing', 'completed')
...
try {
    $order_info = $database->fetch(
        "SELECT o.id, o.order_number, o.status, o.customer_id, o.supplier_id,
    ...
    // Payment status column may not exist in production; relying on status only
