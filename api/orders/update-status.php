...
try {
    ...
    if ($notification_title && $notification_message) {
        ...
        if ($customer) {
            ...
            $notification_sql = "INSERT INTO notifications (user_id, title, message, type, created_at) VALUES (?, ?, ?, 'order', ?)";
            $database->query($notification_sql, [$customer['user_id'], $notification_title, $notification_message, $manila_time]);
        }
    }
